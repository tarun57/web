...
if(isset($_POST['submit'])){
 ...
 $result=mysql_query("SELECT * FROM login2 WHERE name='$email' AND password='$password'");
 $row=mysql_fetch_assoc($result);
  $count=mysql_num_rows($result);
  if ($count==1) {
	$_SESSION['name'] = $row['name'];
echo "<script>alert('Welcome  ". $_SESSION['name'] ."');</script>";
	echo '<script>window.location="question.php?id='.$course_id.'"</script>';
	} else {
  echo "<script>alert('You are not registered Click on SIGN UP?');</script>";
}
}
?>
<?php
...
if(isset($_POST['submit'])){
	$email = $_POST['name'];
	$password = $_POST['password'];
	$query = mysql_query("INSERT INTO `login`( `name`, `password`) VALUES ('$email' ,'$password')");
